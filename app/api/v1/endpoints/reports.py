...
def _export_response(fmt: str, data: bytes, filename: str) -> Response:
    if fmt == "pdf":
        return Response(content=data, media_type="application/pdf", headers={"Content-Disposition": f'attachment; filename="{filename}.pdf"'})
    return Response(content=data, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", headers={"Content-Disposition": f'attachment; filename="{filename}.xlsx"'})
...
async def report_ingresos(
    ...
    if format == "json":
        return [{"id": d.id, "number": d.number, "status": d.status.value, "created_at": d.created_at.isoformat()} for d in docs]

    headers = ["Número", "Estado", "Referencia", "Fecha", "Creado por"]
    rows = [[d.number, d.status.value, d.reference or "", d.created_at.strftime("%Y-%m-%d"), d.created_by] for d in docs]
    title = f"Reporte de Ingresos — {date_from.date()} a {date_to.date()}"
    company = await CompanyRepository(db).get()
    ch = build_header(company, title) if company else None

    if format == "pdf":
        data = ExportService.to_pdf(headers, rows, title, date_from, date_to, current_user.username, company_header=ch)
...
async def report_egresos(
    ...
    rows = [[d.number, d.status.value, d.reference or "", d.created_at.strftime("%Y-%m-%d")] for d in docs]
...
async def report_bajas(
    ...
    rows = [[d.number, d.status.value, d.notes or "", d.created_at.strftime("%Y-%m-%d")] for d in docs]
    title = f"Reporte de Bajas — {date_from.date()} a {date_to.date()}"
    company = await CompanyRepository(db).get()
    ch = build_header(company, title) if company else None
    if format == "pdf":
        return _export_response("pdf", ExportService.to_pdf(headers, rows, title, date_from, date_to, current_user.username, company_header=ch), "reporte_bajas")
    return _export_response("excel", ExportService.to_excel(headers, rows, title, company_header=ch), "reporte_bajas")
...
async def report_ajustes(
    ...
    docs = list(result.scalars().unique().all())
    if format == "json":
        return [{"id": d.id, "number": d.number, "status": d.status.value, "adjust_type": d.adjust_type.value if d.adjust_type else None, "created_at": d.created_at.isoformat()} for d in docs]
    headers = ["Número", "Estado", "Tipo", "Fecha"]
    rows = [[d.number, d.status.value, d.adjust_type.value if d.adjust_type else "", d.created_at.strftime("%Y-%m-%d")] for d in docs]
    title = f"Reporte de Ajustes — {date_from.date()} a {date_to.date()}"
    company = await CompanyRepository(db).get()
    ch = build_header(company, title) if company else None
    if format == "pdf":
        return _export_response("pdf", ExportService.to_pdf(headers, rows, title, date_from, date_to, current_user.username, company_header=ch), "reporte_ajustes")
    return _export_response("excel", ExportService.to_excel(headers, rows, title, company_header=ch), "reporte_ajustes")
...
async def report_stock(
    ...
    if format == "json":
        return [{"id": p.id, "name": p.name, "stock_actual": float(p.stock_actual), "stock_minimo": float(p.stock_minimo), "bajo_stock": p.stock_actual <= p.stock_minimo, "pvp": float(p.pvp)} for p in products]
    headers = ["ID", "Nombre", "Stock Actual", "Stock Mínimo", "Bajo Stock", "PVP"]
    rows = [[p.id, p.name, float(p.stock_actual), float(p.stock_minimo), "Sí" if p.stock_actual <= p.stock_minimo else "No", float(p.pvp)] for p in products]
    title = "Reporte de Stock Actual"
    company = await CompanyRepository(db).get()
    ch = build_header(company, title) if company else None
    if format == "pdf":
        return _export_response("pdf", ExportService.to_pdf(headers, rows, title, None, None, current_user.username, company_header=ch), "reporte_stock")
    return _export_response("excel", ExportService.to_excel(headers, rows, title, company_header=ch), "reporte_stock")
...
async def report_stock_valorizado(
    ...
    for p in products:
        ...
        last_kardex_q = last_kardex_q.order_by(KardexEntry.created_at.desc(), KardexEntry.id.desc()).limit(1)
        ...
        value = stock * cost
        total += value
        items.append({"id": p.id, "name": p.name, "stock": float(stock), "cost": float(cost), "value": float(value), "category_id": p.category_id})
    ...
    ch = build_header(company, title) if company else None
    if format == "pdf":
        return _export_response("pdf", ExportService.to_pdf(headers, rows, title, None, None, current_user.username, company_header=ch), "inventario_valorizado")
    return _export_response("excel", ExportService.to_excel(headers, rows, title, company_header=ch), "inventario_valorizado")
...
async def report_movimientos_por_usuario(
    ...
    if format == "json":
        return [{"id": d.id, "number": d.number, "doc_type": d.doc_type.value, "status": d.status.value, "created_at": d.created_at.isoformat()} for d in docs]
    headers = ["Número", "Tipo", "Estado", "Fecha"]
    rows = [[d.number, d.doc_type.value, d.status.value, d.created_at.strftime("%Y-%m-%d")] for d in docs]
    title = f"Movimientos del Usuario {user_id}"
    company = await CompanyRepository(db).get()
    ch = build_header(company, title) if company else None
    if format == "pdf":
        return _export_response("pdf", ExportService.to_pdf(headers, rows, title, date_from, date_to, current_user.username, company_header=ch), "movimientos_usuario")
    return _export_response("excel", ExportService.to_excel(headers, rows, title, company_header=ch), "movimientos_usuario")
...
async def report_consolidado(
    ...
    total_products = await db.execute(select(func.count(Product.id)).where(Product.status == "active"))
    bajo_stock_count = await db.execute(select(func.count(Product.id)).where(Product.status == "active", Product.stock_actual <= Product.stock_minimo))
