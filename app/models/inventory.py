...
class InventoryDocument(Base):
    ...
    status: Mapped[DocumentStatus] = mapped_column(Enum(DocumentStatus), nullable=False, default=DocumentStatus.pending, index=True)
    reference: Mapped[str | None] = mapped_column(String(200), nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    adjust_type: Mapped[AdjustType | None] = mapped_column(Enum(AdjustType), nullable=True)
    ...
    lines: Mapped[list["InventoryDocumentLine"]] = relationship(back_populates="document", cascade="all, delete-orphan")
    authorization_codes: Mapped[list["AuthorizationCode"]] = relationship(back_populates="document", cascade="all, delete-orphan")
...
class InventoryDocumentLine(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    document_id: Mapped[int] = mapped_column(ForeignKey("inventory_documents.id", ondelete="CASCADE"), nullable=False, index=True)
...
class AuthorizationCode(Base):
    ...
    document: Mapped["InventoryDocument"] = relationship(back_populates="authorization_codes")
